...
Beta  = atan(V(2)/(sqrt(V(1)^2 + V(2) + V(3)^2)));  %Sideslip angle
...
u3 =  0;                           %Control input for ballast mass (constant)
   ...
   F_ext = [-D_1 SF -L]';                       %Total Hydrodynamic force in flow frame
...
rr = rp1_d*N + Rr*(cos(Gamma_d +  pi/2))*E + Rr*(sin(Gamma_d + pi/2)*D);
...
M = [M11 M12;                %Generalized inertia matrix
 ...
 %Change rate of translational and angular momentum (force)
 P_dot = cross(P, omega) + mb*g*R_2'*D + R_2'*F1; 
 P_ang_dot = cross(P_Ang, omega) + cross(P, V) + cross((mrb*rrb + mp*rr)*g, R_2'*D) + R_2'*T1; 
